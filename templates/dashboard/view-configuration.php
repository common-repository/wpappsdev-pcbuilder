...
<?php if ( ! empty( $items ) ) { ?>
	<table class="woocommerce-orders-table woocommerce-MyAccount-orders shop_table shop_table_responsive account-orders-table wpappsdev-view-configuration">
		<thead>
			<tr>
					...
					<?php foreach ( $columns as $column_id => $column_name ) { ?>
						<td class="woocommerce-orders-table__cell woocommerce-orders-table__cell-<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
							...
							switch ( $column_id ) {
								...
								case 'info':
									echo '<div class="wpappsdev-configuration-content">';
									echo wp_kses( $_product->get_image(), wpadpcbu_allowed_html() );
									echo '<div class="wpappsdev-configuration-info">';
...
<style>
	...
	table.wpappsdev-view-configuration {
		font-size: 1em;
	}
	table.wpappsdev-view-configuration tr td{
	...
	.wpappsdev-configuration-content {
		display: flex;
		column-gap: 15px;
		align-items: center;
	}
	.wpappsdev-configuration-content img {
		width: 100px;
	}
</style>
