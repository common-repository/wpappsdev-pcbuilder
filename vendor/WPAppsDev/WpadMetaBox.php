...
class WpadMetaBox {
	...
	public function admin_enqueue_scripts() {
		...
		if ( in_array( $pagenow, [ 'post-new.php', 'post.php' ] ) && in_array( get_post_type(), $this->page ) ) {
			// js
			$deps = [];
			...
			if ( in_array( true, [
				...
				wp_register_script( 'chosen', plugin_dir_url( __FILE__ ) . '/assets/js/chosen.js', [ 'jquery' ] );
				$deps[] = 'chosen';
			...
			if ( in_array( true, [
				...
				self::meta_box_find_field_type( 'chosen', $this->fields ),
				self::meta_box_find_field_type( 'post_chosen', $this->fields ),
				self::meta_box_find_repeatable( $this->fields, 'repeatable' ),
	...
	public function admin_head() {
		...
		if ( in_array( $pagenow, [ 'post-new.php', 'post.php' ] ) && in_array( get_post_type(), $this->page ) && ( self::meta_box_find_field_type( 'date', $this->fields ) || self::meta_box_find_field_type( 'slider', $this->fields ) ) ) {
	...
	public function add_box() {
		foreach ( $this->page as $page ) {
			add_meta_box( $this->id, $this->title, [ $this, 'meta_box_callback' ], $page, $this->context, $this->priority );
		}
	}
	...
	public function meta_box_callback() {
		...
		foreach ( $this->fields as $field ) {
			if ( 'section' == $field['type'] ) {
				...
				$repeatable = 'repeatable' == $field['type'] ? true : false;
	...
	public function save_box( $post_id ) {
		...
		foreach ( $this->fields as $field ) {
			...
			if ( in_array( $field['type'], [ 'tax_select', 'tax_checkboxes' ] ) ) {
				// save taxonomies
	 ...
	 */
	public function meta_box_array_map_r( $func, $meta, $sanitizer ) {
		$newMeta = [];
	 ...
	 */
	public static function meta_box_find_repeatable( $haystack, $needle = 'repeatable' ) {
	 ...
	 */
	public function meta_box_santitize_boolean( $string ) {
		if ( ! isset( $string ) || 1 != $string || true != $string ) {
			return false;
		}
