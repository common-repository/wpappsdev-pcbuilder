...
class WpadCpt extends WpadHelper {
	 ...
	 */
	public function __construct( $post_type, $singular = '', $plural = '', $args = [], $labels = [] ) {
	...
	public function register_post_type() {
		// Default labels.
		$labels = [
			'name'               => sprintf( __( '%s', 'wpappsdev-core' ), $this->plural ),
			'singular_name'      => sprintf( __( '%s', 'wpappsdev-core' ), $this->singular ),
			'menu_name'          => sprintf( __( '%s', 'wpappsdev-core' ), $this->plural ),
			'all_items'          => sprintf( __( '%s', 'wpappsdev-core' ), $this->plural ),
			'add_new'            => __( 'Add New', 'wpappsdev-core' ),
			'add_new_item'       => sprintf( __( 'Add New %s', 'wpappsdev-core' ), $this->singular ),
			'edit_item'          => sprintf( __( 'Edit %s', 'wpappsdev-core' ), $this->singular ),
			'new_item'           => sprintf( __( 'New %s', 'wpappsdev-core' ), $this->singular ),
			'view_item'          => sprintf( __( 'View %s', 'wpappsdev-core' ), $this->singular ),
			'search_items'       => sprintf( __( 'Search %s', 'wpappsdev-core' ), $this->plural ),
			'not_found'          => sprintf( __( 'No %s found', 'wpappsdev-core' ), $this->plural ),
			'not_found_in_trash' => sprintf( __( 'No %s found in Trash', 'wpappsdev-core' ), $this->plural ),
	 ...
	 * @version 1.0.0
	 */
	public function add_taxonomy( $taxonomy_name, $singular = '', $plural = '', $args = [], $labels = [] ) {
	 ...
	 * @return string
	 */
	public function featured_image_content( $content ) {
		$content = str_replace( __( 'Set featured image', 'wpappsdev-core' ), 'Add Image', $content );

		return str_replace( __( 'Remove featured image', 'wpappsdev-core' ), 'Remove Image', $content );
	}
