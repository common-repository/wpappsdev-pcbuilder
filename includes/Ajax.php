...
class Ajax {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_remove_configuration', [ $this, 'remove_configuration_process' ] );
		add_action( 'wp_ajax_nopriv_remove_configuration', [ $this, 'remove_configuration_process' ] );
		...
		// Pc builder search page ajax filtering system for component products.
		add_action( 'wp_ajax_generate_share_link', [ $this, 'generate_share_link_process' ] );
		add_action( 'wp_ajax_nopriv_generate_share_link', [ $this, 'generate_share_link_process' ] );
	}
	...
	public function generate_component_filters_field() {
		...
		foreach ( $filter_items as $item ) {
			$taxonomy = get_filter_taxonomy_slug( $item['filter_slug'] );
	...
	public function add_components_product_to_cart_process() {
		...
		if ( empty( $items ) ) {
			wp_send_json_error(
				[
					'type'    => 'error',
					'message' => __( 'Please select components product.', 'wpappsdev-pcbuilder' ),
				]
			);
		...
		// Required component validations.
		self::component_validations( $items );
		...
		if ( wpadpcbu_process()->builder->redirect_checkout_enable() ) {
			$redirect = true;
		} else {
			$redirect = false;
		}
	...
	public function pcbuilder_configuration_save_process() {
		...
		foreach ( $items as $key => $item ) {
			$pc_config[ $key ] = $item['id'];
		}

		$saved_data = [
			'user_id'        => (int) $user_id,
			'configurations' => $pc_config,
		];

		// Pc builder data save process.
		$saved_id = wpadpcbu_process()->configurations->create( $saved_data );
		...
		wp_send_json_success(
			[
				'url' => wc_get_account_endpoint_url( 'saved-configurations' ),
				'id'  => $saved_id,
			]
		);
		wp_die();
	 ...
	 * @return void
	 */
	public function remove_configuration_process() {
		$post_data = wc_clean( $_POST );
		$nonce     = isset( $post_data['_nonce'] ) ? wc_clean( $post_data['_nonce'] ) : '';
		...
		$saved_id = wpadpcbu_process()->configurations->delete( $saved_id );
	 ...
	 * @return void
	 */
	public function generate_share_link_process() {
		$post_data = wc_clean( $_POST );
		...
		$build_id = isset( $post_data['buildId'] ) ? abs( $post_data['buildId'] ) : 0;
		$nonce    = isset( $post_data['_nonce'] ) ? wc_clean( $post_data['_nonce'] ) : '';
		...
		if ( ! $build_id ) {
			wp_send_json_error(
				[
					'type'    => 'error',
					'message' => __( 'Builder configuration id mission.', 'wpappsdev-pcbuilder' ),
				]
			);
		...
		$args = [
			'id'        => $build_id,
			'share_key' => md5( "{$customer}-{$build_id}-{$nonce}" ),
		];

		// Pc builder data remove process.
		$updated = wpadpcbu_process()->configurations->update( $args );

		// Return error if update process unsuccessful.
		if ( is_wp_error( $updated ) ) {
			wp_send_json_error(
				[
					'type'    => 'error',
					'message' => __( 'Something wrong when update configuration', 'wpappsdev-pcbuilder' ),
				]
			);
		...
		$link = add_query_arg( [ 'share_key' => $args['share_key'] ], trailingslashit( site_url() ) );
	...
	public function filter_component_product_process() {
		...
		foreach ( $temp_filters as $filter ) {
			$tax_slug                   = get_filter_taxonomy_slug( $filter['tax'] );
			$tax_filters[ $tax_slug ][] = $filter['val'];
		}
	...
	public static function validations( $post_data ) {
		...
		do_action( '$hook_name:string', $post_data );
	}

	public static function component_validations( $items ) {
		$missing    = [];
		$components = get_required_component_list();

		foreach ( $components as $cid ) {
			if ( ! isset( $items[ "CI{$cid}" ] ) ) {
				$missing[] = $cid;
			}
		}
		...
		if ( count( $missing ) ) {
			wp_send_json_error(
				[
					...
					'missing' => $missing,
					'message' => __( 'Please select required components.', 'wpappsdev-pcbuilder' ),
				]
			);
			wp_die();
		}
