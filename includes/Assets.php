...
class Assets {
	...
	public function enqueue_front_scripts() {
		...
		$configurations   = isset( $query_vars['saved-configurations'] ) ? true : false;
		$viewpc    = isset( $query_vars['view-configuration'] ) ? true : false;

		do_action( 'wpadpcbu_before_public_enqueue' );

		if ( ! in_array( $object_id, $pages_id, true ) && ! $configurations && ! $viewpc ) {
			return;
		}
		$default_script = [
			'ajaxurl'     => admin_url( 'admin-ajax.php' ),
			...
			'start_range' => wpadpcbu_process()->search->start_range(),
			'end_range'   => wpadpcbu_process()->search->end_range(),
		];
