...
class DynamicTaxonomy {
	...
	public function create_dynamic_taxonomy() {
		...
		foreach ( $taxonomies as $taxonomy ) {
			...
			$tax_slug = get_filter_taxonomy_slug( $taxonomy['filter_slug'] );

			if ( str_starts_with( $tax_slug, 'cf-' ) ) {
				$this->component_filters[] = new WpadTaxonomy( $tax_slug, 'product', $taxonomy['filter_name'], $taxonomy['filter_name'], $args );
			}
		}
	}
}
